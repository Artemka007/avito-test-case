import { cn } from '@/shared/lib/cn';

import type { AdCardVariant } from './types';

type ImagePlaceholderProps = {
  variant: AdCardVariant;
};

export const ImagePlaceholder: React.FC<ImagePlaceholderProps> = ({
  variant,
}) => {
  return (
    <div
      role="img"
      aria-label="Изображение отсутствует"
      className={cn(
        'flex shrink-0 items-center justify-center bg-[var(--bg)] text-[var(--text-muted)]',
        variant === 'grid' && 'aspect-[3/2] w-full',
        variant === 'list' && 'h-[72px] w-[72px] rounded-lg',
      )}
    >
      <svg
        width="full"
        height="full"
        viewBox="0 0 200 150"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
      >
        <path
          d="M0 0H200V142.006C200 146.424 196.418 150.006 192 150.006H8C3.58173 150.006 0 146.424 0 142.006V0Z"
          fill="#FAFAFA"
        />
        <path
          d="M0 0H200V142.006C200 146.424 196.418 150.006 192 150.006H8C3.58173 150.006 0 146.424 0 142.006V0Z"
          fill="url(#pattern0_20_1265)"
        />
        <defs>
          <pattern
            id="pattern0_20_1265"
            patternContentUnits="objectBoundingBox"
            width="1"
            height="1"
          >
            <use
              xlinkHref="#image0_20_1265"
              transform="scale(0.00125005 0.00166667)"
            />
          </pattern>
          <image
            id="image0_20_1265"
            width="800"
            height="600"
            preserveAspectRatio="none"
            xlinkHref="data:image/png;base64,"
          />
        </defs>
      </svg>
    </div>
  );
};
